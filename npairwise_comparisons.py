...
def seek_queue_pair(hashes, outdir):
    ...
    enqueue_threads = [threading.Thread(target=check_batch_pair, args=(sess, hashes, enqueue_pair_op, init_i, batch_size, queue_hash_i, queue_hash_j))
    for init_i in range(last_index, len_hashes, batch_size)]
